...
7. When you're done, review your own work and raise anything questionable. Before handing back, look over the design decisions and logic you just produced, flag the ones that could be wrong, risky, or worth a second opinion, and ask the user about them rather than leaving them silently in place. Prioritise by the conflict-priority order: anything that could leak data, lose data, or corrupt state gets raised first; cosmetic or low-stakes choices can just be stated. Better to ask one good question at the end than ship a confident guess.
